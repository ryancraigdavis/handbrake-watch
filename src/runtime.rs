...
pub async fn run(cfg: ResolvedConfig) -> Result<()> {
    ...
    tokio::spawn(dispatch(cfg.clone(), queue.clone(), tracked.clone(), rx));

    let (sd_tx, sd_rx) = watch::channel(false);
    spawn_signal(sd_tx);
    let notifier = Notifier::new(cfg.notifications.clone());

    info!(folders = cfg.folders.len(), "hbwatch started");
    worker_loop(cfg.clone(), queue, tracked, notifier, sd_rx).await;
...
async fn dispatch(cfg: Arc<ResolvedConfig>, queue: Arc<Queue>, tracked: Tracked, mut rx: Receiver<Candidate>) {
    while let Some(candidate) = rx.recv().await {
        ...
        tokio::spawn(stabilize(cfg.clone(), queue.clone(), tracked.clone(), candidate));
    }
}
...
fn extension_allowed(cfg: &ResolvedConfig, path: &Path) -> bool {
    match path.extension().and_then(|e| e.to_str()) {
        Some(ext) => {
            ...
            cfg.settings.extensions.iter().any(|e| e.eq_ignore_ascii_case(&lower))
        }
        None => false,
    }
}
...
fn is_ignored_name(path: &Path) -> bool {
    ...
    name.starts_with('.')
        || [".tmp", ".part", ".partial", ".download", ".crdownload", ".hbtmp"]
            .iter()
...
pub fn output_path_for(folder: &ResolvedFolder, input: &Path) -> PathBuf {
    ...
    folder.output_dir.join(format!("{stem}.{}", folder.output_extension))
}

/// Wait for the file to stop changing, then enqueue it.
async fn stabilize(cfg: Arc<ResolvedConfig>, queue: Arc<Queue>, tracked: Tracked, candidate: Candidate) {
    ...
    info!(folder = %folder.name, file = %candidate.path.display(), "enqueued");
    queue.push(job);
}
...
async fn worker_loop(
    ...
    while !*shutdown.borrow() {
        match queue.take_next() {
            None => {
                fire_drain_if_needed(&notifier, &mut batch_count, &mut batch_start).await;
                wait_for_work(&queue, &mut shutdown).await;
            }
            Some(job) => {
                batch_start.get_or_insert_with(Instant::now);
                let done = run_one(&cfg, &queue, &tracked, &notifier, &job, &mut shutdown).await;
                if !done {
                    break;
                }
                batch_count += 1;
...
async fn run_one(
    ...
    tokio::select! {
        result = process_job(cfg, job) => {
            queue.complete();
            release(tracked, &job.input_path);
            report(notifier, job, result).await;
            true
        }
        _ = shutdown.changed() => {
...
async fn report(notifier: &Notifier, job: &Job, result: Result<()>) {
    match result {
        Ok(()) => {
            info!(job = %job.id, "encode complete");
...
async fn fire_drain_if_needed(notifier: &Notifier, count: &mut u64, start: &mut Option<Instant>) {
    if *count > 0 {
...
async fn process_job(cfg: &ResolvedConfig, job: &Job) -> Result<()> {
    let temp = mover::temp_path(&job.output_path);
    let result = encode_and_move(cfg, job, &temp).await;
    if result.is_err() {
...
async fn encode_and_move(cfg: &ResolvedConfig, job: &Job, temp: &Path) -> Result<()> {
    encoder::run(&cfg.settings, job, temp).await?;
